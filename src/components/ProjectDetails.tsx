...
interface ProjectDetailsProps {
    ...
    prevProject: ProjectNavInfo;
    nextProject: ProjectNavInfo;
}

export default function ProjectDetails({ project, prevProject, nextProject }: ProjectDetailsProps) {
                ...
                <div className="container px-4 mx-auto">
                    ...
                    <div className="flex flex-col sm:flex-row gap-6">
                        <Link
                            ...
                            className="group flex flex-1 items-center gap-4 bg-slate-200 dark:bg-slate-700 rounded-lg shadow-lg hover:shadow-xl transition duration-300 transform hover:scale-105 p-5 min-w-0"
                        >
                            <span className="flex items-center justify-center w-12 h-12 rounded-full bg-primary text-white shrink-0 transition-transform duration-300 group-hover:-translate-x-1">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                                </svg>
                            </span>
                            <span className="min-w-0">
                                <span className="block text-sm font-semibold text-primary">Previous Project</span>
                                <span className="block font-bold text-dark dark:text-white truncate">{prevProject.title}</span>
                            </span>
                        </Link>
                        <Link
                            href={`/portfolio/${nextProject.id}`}
                            className="group flex flex-1 items-center justify-end gap-4 bg-slate-200 dark:bg-slate-700 rounded-lg shadow-lg hover:shadow-xl transition duration-300 transform hover:scale-105 p-5 min-w-0"
                        >
                            <span className="min-w-0 text-right">
                                <span className="block text-sm font-semibold text-primary">Next Project</span>
                                <span className="block font-bold text-dark dark:text-white truncate">{nextProject.title}</span>
                            </span>
                            <span className="flex items-center justify-center w-12 h-12 rounded-full bg-primary text-white shrink-0 transition-transform duration-300 group-hover:translate-x-1">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
